...
// Report가 없는 경우도 포함
    ...
    List<CommentsComment> content = queryFactory
            .selectFrom(QCommentsComment.commentsComment)
            .leftJoin(QReport.report).on(QCommentsComment.commentsComment.commentsCommentId.eq(QReport.report.commentsComment.commentsCommentId).and(QReport.report.reportStatus.eq(false)))
            .where(searchByLike(commentSearchDto.getSearchBy(),commentSearchDto.getSearchQuery()))
